...
const blankRow = () => ({
  crop: '', customCropName: '', quantity_sold_kg: '', quantity_discarded_kg: '',
  discard_reason: 'SPOILAGE', discard_notes: '',
})

function rowWasteRate(row) {
  const sold = parseFloat(row.quantity_sold_kg)
...
export default function WasteReportPage() {
  ...
  useEffect(() => {
    cooperativesApi.getCrops()
      ...
      .catch(() => {})
    marketAgentApi.getWasteReports()
  ...
  const onSubmit = async (e) => {
    ...
    for (const [i, row] of rows.entries()) {
      ...
      const sold = parseFloat(row.quantity_sold_kg)
      const disc = parseFloat(row.quantity_discarded_kg)
      if (isNaN(sold) || sold < 0) { toast.error(`Row ${i + 1}: enter quantity sold`); return }
    ...
    setLoading(true)
    try {
      const res = await marketAgentApi.submitWasteReportBatch({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Record, per crop, end-of-day or end-of-week market spoilage.</p>
      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="number" min="0" step="0.1" className="input" placeholder="0"
                        value={row.quantity_sold_kg} onChange={e => updateRow(i, 'quantity_sold_kg', e.target.value)} required />
                    </div>
                    <div>
