...
export default function WasteReportPage() {
  ...
  const wasteRate = () => {
    const moved = parseFloat(form.quantity_moved_kg)
  ...
  const onSubmit = async (e) => {
    ...
    const moved = parseFloat(form.quantity_moved_kg)
    const disc = parseFloat(form.quantity_discarded_kg)
    if (isNaN(moved) || moved < 0) { toast.error('Enter quantity moved on'); return }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Record produce that spoiled or was discarded while in your warehouse, before it reached a market agent.</p>
      </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="0"
                value={form.quantity_moved_kg}
                onChange={e => setForm(f => ({ ...f, quantity_moved_kg: e.target.value }))}
                required
              />
          ...
          {wasteRate() !== null && (
            <div className={`rounded-lg px-4 py-2 text-sm font-medium flex items-center gap-2 ${
              ...
              Warehouse spoilage rate: {wasteRate()}%
            </div>
          )}
