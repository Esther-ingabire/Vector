...
const REASONS = [
  ...
  { value: 'NO_DEMAND', label: 'No demand — not collected before spoilage' },
  { value: 'DAMAGE',    label: 'Physical/handling damage in warehouse' },
...
const blankRow = () => ({
  crop: '', customCropName: '', quantity_moved_kg: '', quantity_discarded_kg: '',
  discard_reason: 'SPOILAGE', discard_notes: '',
})

function rowWasteRate(row) {
  const moved = parseFloat(row.quantity_moved_kg)
  const disc = parseFloat(row.quantity_discarded_kg)
  if (!isNaN(moved) && !isNaN(disc) && (moved + disc) > 0) {
    return ((disc / (moved + disc)) * 100).toFixed(1)
  }
  return null
}
...
export default function WasteReportPage() {
  ...
  // Real "quantity moved on" per crop for the selected period, from confirmed market-agent
  // collections — so this form reports against what the system actually recorded, instead
  // of the distributor guessing from memory. { [cropId]: kg }
  ...
  useEffect(() => {
    cooperativesApi.getCrops()
      ...
      .catch(() => {})
    distributionApi.getMyWasteReports()
  ...
  useEffect(() => {
    ...
    distributionApi.getSoldSummary({ period_start: periodStart, period_end: periodEnd })
      .then(res => {
        const map = {}
        for (const r of (res.data || [])) map[r.crop_id] = Number(r.moved_kg)
        setSoldSummary(map)
      })
      .catch(() => setSoldSummary({}))
  ...
  const onSubmit = async (e) => {
    ...
    for (const [i, row] of rows.entries()) {
      ...
      const moved = parseFloat(row.quantity_moved_kg)
      const disc = parseFloat(row.quantity_discarded_kg)
      if (isNaN(moved) || moved < 0) { toast.error(`Row ${i + 1}: enter quantity moved on`); return }
    ...
    setLoading(true)
    try {
      const res = await distributionApi.submitWasteReportBatch({
        ...
        rows: rows.map(row => {
          ...
          return {
            ...
            quantity_moved_kg: Number(row.quantity_moved_kg),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Record, per crop, produce that spoiled or was discarded while in your warehouse, before it reached a market agent.</p>
      </div>

      <div className="card max-w-3xl">
        <h2 className="text-base font-semibold text-gray-700 mb-5">Record Warehouse Waste</h2>
              ...
              return (
                <div key={i} className="p-4 bg-gray-50 rounded-xl space-y-3">
                  ...
                  <div>
                    ...
                    <select className="input" value={row.crop} onChange={e => {
                      const cropId = e.target.value
                      updateRow(i, 'crop', cropId)
                      const suggested = soldSummary[cropId]
                      if (suggested != null && !row.quantity_moved_kg) {
                        updateRow(i, 'quantity_moved_kg', String(suggested))
                      }
                    }} required>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="label">Quantity moved on (kg)</label>
                      <input type="number" min="0" step="0.1" className="input" placeholder="0"
                        value={row.quantity_moved_kg} onChange={e => updateRow(i, 'quantity_moved_kg', e.target.value)} required />
                      {soldSummary[row.crop] != null ? (
                        <p className="text-xs text-gray-500 mt-1 flex items-center gap-1 flex-wrap">
                          ...
                          {soldSummary[row.crop].toLocaleString()} kg confirmed collected this period
                          {Number(row.quantity_moved_kg) !== soldSummary[row.crop] && (
                            <button type="button" className="text-primary-600 hover:underline font-medium"
                              onClick={() => updateRow(i, 'quantity_moved_kg', String(soldSummary[row.crop]))}>
                              Use this figure
                            </button>
                          )}
                        </p>
                      ) : (
                        <p className="text-xs text-gray-400 mt-1">Sold or collected by market agents in this period</p>
                      )}
                    </div>
                    <div>
              ...
              <div key={r.id ?? i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    Moved on: {r.quantity_moved_kg} kg · Discarded: {r.quantity_discarded_kg} kg
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {r.warehouse_spoilage_loss_pct != null && (
                    <span className={`text-sm font-bold ${parseFloat(r.warehouse_spoilage_loss_pct) > 10 ? 'text-warning-500' : 'text-success-600'}`}>
                      {r.warehouse_spoilage_loss_pct}% waste
                    </span>
                  )}
